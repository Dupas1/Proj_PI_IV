...
public class PreferencesController {
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody PreferencesRequest preferencesRequest, BindingResult bindingResult){
        if(bindingResult.hasErrors()) return handleErrors(bindingResult);
        return ResponseEntity.ok(service.create(preferencesRequest));
    }
    @GetMapping("/{uid}")
    ...
    @PutMapping("/{uid}")
    public ResponseEntity<?> udpate(@PathVariable String uid, @Valid @RequestBody PreferencesRequest request, BindingResult bindingResult){
        if(bindingResult.hasErrors()) return handleErrors(bindingResult);
